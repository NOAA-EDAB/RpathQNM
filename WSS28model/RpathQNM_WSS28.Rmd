...
for(j in seq_along(q.models)){
  ...
  while(i < n.sims) {
    ...
    save(results, paste0("WSS28","results", j,"Small crab other artopoda_down",".RData"))
...
for(j in seq_along(q.models)){
  ...
  while(i < n.sims) {
    ...
    rownames(results) <- levels(q.models[[j]]$From)
    colnames(results) <- c('-','0','+')
    save(results, paste0("WSS62","results", j,"Small crab other artopoda_down",".RData"))
    write.csv(results, paste0("WSS62","results", j,"Small crab other artopoda_down",".csv"))
    ...
    prop <- results/rowSums(results)
    r <- colSums(t(prop)*(-1:1))
    filename= paste("WSS62", "plots", j,"Small crab other artopoda_down", ".pdf")
    pdf(filename)
    #adjust barplot if you want it ordered by proportion
    par(mar=c(5,10,1,1)+0.1)
    barplot(t(prop),
