...
data<-WSS28qnm100
WSS28qnm100<-MM2Qpress(data)
WSS28qnm100<-dput(WSS28qnm100)
WSS28qnm100<-parse.digraph(WSS28qnm100)
...
A <- adjacency.matrix(WSS28qnm100el)
#A
...
#                        monitor=c("Commercial_Pelagic_Fishery"=1))
#Press/Perturbation with decrease forage fish and no monitoring)
press <- press.validate(WSS28qnm100el,
                        perturb=c("Fishery"=-1), monitor=F)

## Function to define the perturbation scenario
impact <- press.impact(WSS28qnm100el,perturb=c("Fishery"=-1))
...
## Print results
rownames(results) <- levels(WSS28qnm100el$From)
colnames(results) <- c('-','0','+')
#results

##save results
WSS28qnm100_Fishery_down<-results
write.csv(WSS28qnm100_Fishery_down, file="WSS28100_Fishery_down.csv")
save(WSS28qnm100_Fishery_down, file="WSS28100_Fishery_down.RData")
...
WSS28_results_ecosQNM<-bind_rows(WSS28_QNM_results_compiled_tr, WSS28.results_ecos)

saveRDS(WSS28_results_ecosQNM, "~/JCT work/WGNARS/RpathtoQNM/RpathQNM/data/WSS28_results_ecosQNM.RDS")

```
## Graphing ecosense and qnm
```{r}
library(reshape2)
library(ggplot2)
library(gridExtra)
...
ecoQNMresults_long<-ecoQNMresults %>%
                    pivot_longer (cols= 7:34, names_to="node" ) %>%
                   mutate(sign=value) %>%
                    mutate(fill=value) 
                    
ecoQNMresults_long$sign[ecoQNMresults_long$sign >600] <- "positive"
ecoQNMresults_long$sign[ecoQNMresults_long$sign >400 & ecoQNMresults_long$fill <=600] <- "zero" 
ecoQNMresults_long$sign[ecoQNMresults_long$sign <=400] <- "negative"

ecoQNMresults_long$fill[ecoQNMresults_long$fill >800] <- "strong_positive"
ecoQNMresults_long$fill[ecoQNMresults_long$fill >600 & ecoQNMresults_long$fill <=800] <- "moderate_positive"
ecoQNMresults_long$fill[ecoQNMresults_long$fill >400 & ecoQNMresults_long$fill <=600] <- "weak"
ecoQNMresults_long$fill[ecoQNMresults_long$fill >200 & ecoQNMresults_long$fill <=400] <- "moderate_negative"
ecoQNMresults_long$fill[ecoQNMresults_long$fill <=200] <- "strong_negative"
