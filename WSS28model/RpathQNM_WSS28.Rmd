...
for(j in seq_along(q.models)){
  ...
  while(i < n.sims) {
    ...
    colnames(results) <- c('-','0','+')
     save(results, file=paste0("WSS28","results_neu", j,"Phytoplankton_up",".RData"))
    write.csv(results, paste0("WSS28","results_neu", j,"Phytoplankton_up",".csv"))
...
dat_smallpelagicsplus<- all.results %>% 
...
dat_smallpelagicsminus<- all.results %>% 
...
dat_phytoplanktonplus<- all.results %>% 
...
dat_phytoplanktonminus<- all.results %>% 
...
jpeg(filename="RpathQNM_combined_plots_neut_mix.jpeg", width=20, height=15, units="in", quality=100,res=600)
